  ...
  <div>
    ...
    <div v-if="!isMobile">
      <div v-if="count === 1" class="grid grid-cols-1 h-max-40">
        <img :src="images?.[0]?.url" class="w-full h-auto object-cover rounded cursor-pointer" @click.stop="$emit('zoom', images[0].url)" />
      </div>

      <div v-else-if="count === 2" class="grid grid-cols-2 gap-1 h-max-40">
        <img v-for="(image, index) in images" :key="index" :src="image.url" class="w-full h-64 object-cover rounded cursor-pointer" @click.stop="$emit('zoom', image.url)" />
      </div>

      <div v-else-if="count === 3" class="grid grid-cols-3 gap-1 h-max-40">
        <img :src="images?.[0]?.url" class="col-span-2 row-span-2 w-full h-full object-cover rounded cursor-pointer" @click.stop="$emit('zoom', images[0].url)" />
        <div class="grid grid-rows-2 gap-1">
          <img v-for="(image, index) in images.slice(1)" :key="index" :src="image.url" class="w-full h-full object-cover rounded cursor-pointer" @click.stop="$emit('zoom', image.url)" />
        </div>
      </div>

      <div v-else-if="count === 4" class="grid grid-cols-2 gap-1 h-max-40">
        <img v-for="(image, index) in images" :key="index" :src="image.url" class="w-full h-48 object-cover rounded cursor-pointer" @click.stop="$emit('zoom', image.url)" />
      </div>
      ...
      <div v-else class="grid grid-cols-3 gap-1 h-max-40">
        <div v-for="(image, index) in visibleImages" :key="index" class="relative h-40 cursor-pointer" @click.stop="$emit('zoom', image.url)">
    ...
    <div v-else
      ...
      <button v-if="currentIndex > 0" @click.stop="currentIndex--"
              class="absolute left-2 top-1/2 transform -translate-y-1/2 bg-black/50 text-white rounded-full p-2">
        ‹
      </button>
      <button v-if="currentIndex < images.length - 1" @click.stop="currentIndex++"
